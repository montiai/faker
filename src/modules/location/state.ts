...
export function state(
  ...
  const stateDataSet = abbreviated
    ? assertLocaleData(
        fakerCore.definitions.location?.state_abbr,
        'location.state_abbr'
      )
    : assertLocaleData(fakerCore.definitions.location?.state, 'location.state');

  return arrayElement(fakerCore, stateDataSet);
}
